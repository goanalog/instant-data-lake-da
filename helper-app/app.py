...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Shimmer skeleton for loading (A) */
  .shimmer {
    position: relative; overflow: hidden; background: var(--surface); border-radius: 8px;
  }
  .shimmer::after {
...
// Load list (with shimmer); auto-open first CSV (B), IBM style accents (C/D)
fetch("/api/files")
  .then(r => r.json())
  .then(files => {
