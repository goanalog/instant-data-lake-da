...
resource "ibm_resource_instance" "cos_instance" {
  ...
  tags              = ["service:cos", "variation:cognos", "prefix:${var.prefix}"] # Tags OK here
}
...
resource "ibm_cos_bucket" "cos_bucket" {
  ...
  storage_class        = "smart"
  force_delete         = true
  # tags                 = ["service:cos-bucket", "variation:cognos", "prefix:${var.prefix}"] # <-- REMOVED THIS LINE
}
...
resource "ibm_resource_instance" "sql_query_instance" {
  ...
  location          = var.region
  resource_group_id = data.ibm_resource_group.group.id
  tags              = ["service:sql-query", "variation:cognos", "prefix:${var.prefix}"] # Tags OK here
...
resource "ibm_resource_instance" "cognos_analytics" {
  ...
  location          = var.region
  resource_group_id = data.ibm_resource_group.group.id
  tags              = ["service:cognos-analytics", "variation:cognos", "prefix:${var.prefix}"] # Tags OK here
}